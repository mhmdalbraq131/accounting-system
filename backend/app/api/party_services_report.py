...
def _entry_rows(db: Session, entry_ids: set[int], party_account_id: int, user: User, from_date: date | None, to_date: date | None, service_type: str):
    ...
    stmt = (
        ...
        .where(JournalEntry.status == "posted", JournalLine.account_id == party_account_id, JournalEntry.id.in_(entry_ids))
    )
    ...
    for line, entry in db.execute(stmt).all():
        rows.append({
            ...
            "debit": line.debit,
            "credit": line.credit,
            "balance_delta": Decimal(str(line.debit or 0)) - Decimal(str(line.credit or 0)),
        })
    return rows
...
def party_services_report(
    ...
    booking_stmt = (
        ...
        .where(
            ...
            TravelProgram.program_type.in_(list(selected)),
        )
    )
    if user.branch_id is not None:
    ...
    order_stmt = select(ServiceOrder.id, ServiceOrder.journal_entry_id, ServiceOrder.service_type).where(
        ServiceOrder.agent_id == party.id,
        ServiceOrder.journal_entry_id.is_not(None),
        ServiceOrder.service_type.in_([x for x in selected if x in {"flight", "bus", "visit", "work_visa"}]),
    )
    if user.branch_id is not None:
    ...
    voucher_stmt = select(Voucher).where(
        ...
        Voucher.source_account_id == party.account_id,
    ...
    for voucher in db.scalars(voucher_stmt.order_by(Voucher.voucher_date, Voucher.id)).all():
        ...
        if voucher.linked_service_type in {"flight", "bus", "visit", "work_visa"}:
            mapped = voucher.linked_service_type
        if mapped is None and voucher.linked_service_type == "service_order":
            order = db.get(ServiceOrder, voucher.linked_service_id)
            mapped = order.service_type if order else None
        ...
        rows.append({
            ...
            "debit": 0,
            "credit": amount,
            "balance_delta": -Decimal(str(amount or 0)),
        })

    rows.sort(key=lambda r: (r["entry_date"], r["entry_number"]))
