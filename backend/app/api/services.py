...
def _party(db: Session, user: User, party_id: int | None, allowed: set[str], label: str, account_type: str) -> Party:
    ...
    if not party or not party.is_active or party.party_type not in allowed:
    ...
    if not account or not account.is_active or account.account_type != account_type:
...
def _account_setting(db: Session, user: User, service_type: str, kind: str) -> Account:
    prefix = SERVICE_LABELS[service_type]
    key = f"{service_type}_{kind}_account_id"
    expected = "revenue" if kind == "revenue" else "expense"
    ...
    if raw:
        try:
            ...
            raise HTTPException(400, f"إعداد {prefix} غير صالح")
    if account is None:
        ...
        if len(candidates) != 1:
            raise HTTPException(400, f"اضبط حساب {prefix} {kind} في الإعدادات")
        account = candidates[0]
    if account.account_type != expected or not account.is_active:
...
def list_services(service_type: str, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    if user.branch_id is not None:
        stmt = stmt.where((ServiceOrder.branch_id == user.branch_id) | ServiceOrder.branch_id.is_(None))
    return list(db.scalars(stmt))


@router.post("", status_code=201)
def create_service(payload: ServiceCreate, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    if payload.service_type not in SERVICE_LABELS:
        raise HTTPException(400, "نوع الخدمة غير مدعوم")
    if db.scalar(select(ServiceOrder).where(ServiceOrder.reference_no == payload.reference_no)):
        raise HTTPException(409, "رقم الخدمة مستخدم مسبقًا")
    if payload.agent_id is None and payload.customer_id is None:
        raise HTTPException(400, "يجب تحديد الوكيل أو العميل المباشر")
    if payload.agent_id is not None and payload.customer_id is not None:
        raise HTTPException(400, "لا يجتمع الوكيل والعميل في نفس الخدمة")
...
def post_service(service_type: str, service_id: int, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    if not row or row.service_type != service_type:
        raise HTTPException(404, "الخدمة غير موجودة")
    _scope(user, row.branch_id)
    if row.status != "draft" or row.journal_entry_id:
        raise HTTPException(400, "الخدمة غير قابلة للترحيل")
    ...
    lines = [
        {"account_id": counterparty.account_id, "debit": row.sale_price, "credit": Decimal("0"), "description": f"استحقاق خدمة {SERVICE_LABELS[service_type]} #{row.id}"},
    ...
    if row.supplier_cost > 0:
        ...
        lines.extend([
            {"account_id": cost_account.id, "debit": row.supplier_cost, "credit": Decimal("0"), "description": f"تكلفة {SERVICE_LABELS[service_type]} #{row.id}"},
...
def cancel_service(service_type: str, service_id: int, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    if row.status == "cancelled":
        raise HTTPException(400, "الخدمة ملغاة مسبقًا")
    if row.journal_entry_id:
        original = db.get(__import__("app.models.journal", fromlist=["JournalEntry"]).JournalEntry, row.journal_entry_id)
        if not original:
            raise HTTPException(409, "القيد المرتبط بالخدمة غير موجود")
        try:
            create_journal(db, entry_number=f"REV-{service_type.upper()}-{row.id}", entry_date=date.today(), description=f"عكس خدمة {SERVICE_LABELS[service_type]} #{row.id}", lines=[{"account_id": line.account_id, "debit": line.credit, "credit": line.debit} for line in original.lines], created_by=user.id, branch_id=row.branch_id, status="posted")
        except ValueError as exc:
            db.rollback()
            raise HTTPException(400, str(exc))
