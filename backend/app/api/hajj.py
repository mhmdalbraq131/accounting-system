...
def _create_service_booking(db: Session, user: User, *, program: TravelProgram, pilgrim: Pilgrim, agent_id: int | None, customer_id: int | None, supplier_id: int, sale_price: Decimal, supplier_cost: Decimal, quota_id: int | None = None) -> ProgramBooking:
    ...
    booking = ProgramBooking(
        program_id=program.id, pilgrim_id=pilgrim.id, customer_id=customer_id, agent_id=agent_id,
        quota_id=quota_id, supplier_id=supplier_id, sale_price=sale_price, supplier_cost=supplier_cost,
        paid_amount=Decimal("0"), remaining_amount=sale_price, profit=sale_price - supplier_cost,
...
def list_quotas(db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    return [{"id": r.id, "name_ar": r.name_ar, "season": r.season, "supplier_id": r.supplier_id, "total_units": r.total_units,
             "used_units": r.used_units, "remaining_units": max(r.total_units-r.used_units,0), "unit_cost": r.unit_cost,
             "status": r.status, "branch_id": r.branch_id} for r in rows]
...
def create_hajj_booking(payload: HajjBookingCreate, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    return _create_service_booking(db, user, program=program, pilgrim=pilgrim, agent_id=payload.agent_id, customer_id=payload.customer_id,
                                   supplier_id=quota.supplier_id, sale_price=payload.sale_price, supplier_cost=cost, quota_id=quota.id)
...
def post_hajj_booking(booking_id: int, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    lines = [{"account_id": counterparty.account_id, "debit": booking.sale_price, "credit": Decimal("0"), "description": f"استحقاق خدمات الحج للحجز #{booking.id}"},
    ...
    if booking.supplier_cost > 0:
        ...
        lines.extend([{ "account_id": cost_account.id, "debit": booking.supplier_cost, "credit": Decimal("0"), "description": f"تكلفة حج للحجز #{booking.id}"},
...
def cancel_hajj_booking(booking_id: int, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    if booking.journal_entry_id:
        ...
        try:
            create_journal(db, entry_number=f"REV-HAJJ-BOOK-{booking.id}", entry_date=date.today(), description=f"عكس حجز الحج #{booking.id}",
