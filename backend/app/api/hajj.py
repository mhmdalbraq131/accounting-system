...
def _party(db: Session, user: User, party_id: int | None, allowed: set[str], label: str, required_account_type: str | None = None) -> Party:
    ...
    if not party or party.party_type not in allowed or not party.is_active: raise HTTPException(400, f"{label} غير موجود أو نوعه غير صحيح")
    ...
    if not account or not account.is_active: raise HTTPException(400, f"حساب {label} غير موجود أو غير نشط")
    if required_account_type and account.account_type != required_account_type: raise HTTPException(400, f"حساب {label} يجب أن يكون من نوع {required_account_type}")
    _scope(user, account.branch_id); return party
...
def _setting_account(db: Session, user: User, key: str, expected_type: str, label: str) -> Account:
    ...
    if raw:
        ...
        except ValueError: raise HTTPException(400, f"إعداد {label} غير صحيح")
    if account is None:
...
def _create_service_booking(db: Session, user: User, *, program: TravelProgram, pilgrim: Pilgrim, agent_id: int | None, customer_id: int | None, supplier_id: int, sale_price: Decimal, supplier_cost: Decimal, quota_id: int | None = None) -> ProgramBooking:
    if agent_id is None and customer_id is None: raise HTTPException(400, "يجب تحديد الوكيل أو العميل المباشر")
    if agent_id is not None and customer_id is not None: raise HTTPException(400, "لا يجتمع الوكيل والعميل المباشر في نفس الخدمة")
    _party(db, user, supplier_id, {"supplier", "both"}, "المورد", "liability")
    if agent_id is not None: _party(db, user, agent_id, {"agent"}, "الوكيل", "asset")
    else: _party(db, user, customer_id, {"customer", "both"}, "العميل", "asset")
...
def list_quotas(db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    return [{"id":r.id,"name_ar":r.name_ar,"season":r.season,"supplier_id":r.supplier_id,"total_units":r.total_units,"used_units":r.used_units,"remaining_units":max(r.total_units-r.used_units,0),"unit_cost":r.unit_cost,"status":r.status,"branch_id":r.branch_id} for r in rows]
...
def create_hajj_booking(payload:HajjBookingCreate,db:Session=Depends(get_db),user:User=Depends(get_current_user)):
    ...
    if not pilgrim:raise HTTPException(404,"الحاج غير موجود")
    _scope(user,pilgrim.branch_id);quota=db.scalar(select(HajjQuota).where(HajjQuota.id==payload.quota_id).with_for_update())
    if not quota:raise HTTPException(404,"حصة الحج غير موجودة")
    _scope(user,quota.branch_id)
    if quota.status!="open":raise HTTPException(409,"حصة الحج مغلقة")
    if quota.used_units>=quota.total_units:raise HTTPException(409,"اكتملت حصة الحجاج لهذا المورد")
    cost=payload.supplier_cost if payload.supplier_cost is not None else quota.unit_cost;quota.used_units+=1
    return _create_service_booking(db,user,program=program,pilgrim=pilgrim,agent_id=payload.agent_id,customer_id=payload.customer_id,supplier_id=quota.supplier_id,sale_price=payload.sale_price,supplier_cost=cost,quota_id=quota.id)
...
def post_hajj_booking(booking_id:int,db:Session=Depends(get_db),user:User=Depends(get_current_user)):
    ...
    supplier=_party(db,user,booking.supplier_id,{"supplier","both"},"مورد الحج","liability");revenue=_setting_account(db,user,"travel_revenue_account_id","revenue","إيراد الحج والعمرة")
    lines=[{"account_id":counterparty.account_id,"debit":booking.sale_price,"credit":Decimal("0"),"description":f"استحقاق خدمات الحج للحجز #{booking.id}"},{"account_id":revenue.id,"debit":Decimal("0"),"credit":booking.sale_price,"description":f"إيراد الحج للحجز #{booking.id}"}]
...
def cancel_hajj_booking(booking_id:int,db:Session=Depends(get_db),user:User=Depends(get_current_user)):
    ...
    if Decimal(str(booking.paid_amount or 0))>0:raise HTTPException(409,"لا يمكن إلغاء خدمة حج عليها تحصيلات؛ اعكس أو ألغِ سندات القبض المرتبطة أولًا")
...
def create_umrah_booking(payload:UmrahBookingCreate,db:Session=Depends(get_db),user:User=Depends(get_current_user)):
    ...
    _scope(user,pilgrim.branch_id);booked=db.scalar(select(func.count(ProgramBooking.id)).where(ProgramBooking.program_id==program.id,ProgramBooking.status=="confirmed")) or 0
    ...
    else:_party(db,user,payload.customer_id,{"customer","both"},"العميل","asset")
...
def cancel_umrah_booking(booking_id:int,db:Session=Depends(get_db),user:User=Depends(get_current_user)):
    ...
    _scope(user,booking.branch_id)
    if booking.status=="cancelled":raise HTTPException(400,"الحجز ملغى مسبقًا")
    if Decimal(str(booking.paid_amount or 0))>0:raise HTTPException(409,"لا يمكن إلغاء خدمة عمرة عليها تحصيلات؛ اعكس أو ألغِ سندات القبض المرتبطة أولًا")
    if booking.journal_entry_id:
        original=db.get(__import__("app.models.journal",fromlist=["JournalEntry"]).JournalEntry,booking.journal_entry_id)
        if not original:raise HTTPException(409,"القيد المرتبط بالحجز غير موجود")
