...
def create_hajj_booking(payload: HajjBookingCreate, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    supplier = _party(db, user, quota.supplier_id, {"supplier", "both"}, "مورد الحج", "liability")
    ...
    booking = ProgramBooking(
        ...
        profit=payload.sale_price - cost,
        customer_type="agency" if payload.agent_id is not None else "direct",
        status="reserved",
        branch_id=user.branch_id,
    )
    db.add(booking)
...
def post_hajj_booking(booking_id: int, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    if booking.status not in {"reserved", "confirmed"} or booking.journal_entry_id:
    ...
    if booking.agent_id is not None:
        counterparty = _party(db, user, booking.agent_id, {"agent"}, "الوكيل", "asset")
    else:
        counterparty = _party(db, user, booking.customer_id, {"customer", "both"}, "العميل", "asset")
    ...
    entry = create_journal(
        db,
        entry_number=f"HAJJ-BOOK-{booking.id}",
        entry_date=booking.booked_at.date(),
        description=f"ترحيل خدمة حج للحجز #{booking.id}",
        lines=lines,
        created_by=user.id,
...
def agent_balance(agent_id: int, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    debit = db.scalar(
        select(func.coalesce(func.sum(ProgramBooking.sale_price), 0)).where(
            ProgramBooking.agent_id == agent.id,
            ProgramBooking.status == "confirmed",
        )
    ) or Decimal("0")
    paid = db.scalar(
        select(func.coalesce(func.sum(ProgramBooking.paid_amount), 0)).where(
            ProgramBooking.agent_id == agent.id,
            ProgramBooking.status == "confirmed",
        )
    ) or Decimal("0")
