...
export async function api<T>(path: string, options: RequestInit = {}): Promise<T> {
  ...
  } catch (error) {\n    const reason = error instanceof Error && error.message ? ` (${error.message})` : "";\n    throw new Error(`تعذر الاتصال بخادم API أثناء طلب ${path}. العنوان: ${API_BASE}${reason}`);\n  }st API_BASE = (import.meta.env.VITE_API_URL || `${window.location.protocol}//${window.location.hostname}:8000/api/v1`).replace(/\/$/, "");
...
export async function api<T>(path: string, options: RequestInit = {}): Promise<T> {
  ...
  try {
    ...
    throw new Error(`تعذر الاتصال بخادم النظام. تأكد أن الواجهة تعمل وأن خادم API متاح على ${API_BASE}`);
  }
  if (!response.ok) {
