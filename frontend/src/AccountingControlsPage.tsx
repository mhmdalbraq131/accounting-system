...
import {closeFiscalPeriod,createAccountingDimension,createFiscalPeriod,disableAccountingDimension,getAccountingDimensions,getAuditLogs,getFiscalPeriods,type AccountingDimension,type AuditLog,type FiscalPeriod} from "./api";
...
export default function AccountingControlsPage(){
  ...
  async function disableDimension(id:number){try{await disableAccountingDimension(id);await load()}catch(e){setError(e instanceof Error?e.message:"تعذر تعطيل البعد")}}
  return <div className="page-stack" dir="rtl">
    ...
    <div className="panel"><div className="panel-head"><h3>الأبعاد المحاسبية</h3></div><div className="table-wrap"><table><thead><tr><th>الرمز</th><th>الاسم</th><th>النوع</th><th>الحالة</th><th></th></tr></thead><tbody>{dimensions.map(d=><tr key={d.id}><td>{d.code}</td><td>{d.name_ar}</td><td>{d.dimension_type}</td><td>{d.is_active?"نشط":"موقوف"}</td><td>{d.is_active&&<button className="secondary" onClick={()=>disableDimension(d.id)}>تعطيل</button>}</td></tr>)}</tbody></table></div></div>
