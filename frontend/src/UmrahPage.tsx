...
const postedStatus=(s:string)=>s==="posted"||s==="confirmed";
...
export default function UmrahPage({programs,pilgrims,suppliers,customers}:{programs:Program[];pilgrims:Pilgrim[];suppliers:any[];customers:any[]}){
  ...
  const [receipt,setReceipt]=useState({booking_id:"",financial_id:"",voucher_number:"",amount:"",date:new Date().toISOString().slice(0,10),description:"تحصيل من خدمة عمرة"});
  ...
  async function collect(e:React.FormEvent){e.preventDefault();setMessage("");setError("");try{
    ...
    const v=await createVoucher({voucher_number:receipt.voucher_number.trim()||undefined,voucher_type:"receipt",voucher_date:receipt.date,amount,description:receipt.description,source_account_id:Number(payer.account_id),destination_account_id:Number(f.ledger_account_id),linked_service_type:"umrah_booking",linked_service_id:selected.id});
    await postVoucher(v.id);setReceipt(r=>({...r, voucher_number:"", amount:""}));setMessage(`تم تحصيل ${money(amount)} وربطه بخدمة العمرة #${selected.id}.`);await refresh();
  ...
  return <main className="page">
    ...
    <div className="stats compact"><Stat label="الحجوزات" value={bookings.length} unit="خدمة"/><Stat label="المرحّلة" value={bookings.filter(b=>postedStatus(b.status)).length} unit="خدمة"/><Stat label="إجمالي البيع" value={money(bookings.reduce((n,b)=>n+Number(b.sale_price),0))} unit="ريال"/><Stat label="إجمالي الربح" value={money(bookings.reduce((n,b)=>n+Number(b.profit),0))} unit="ريال"/></div>
    ...
    <section className="panel"><div className="panel-head"><div><h3>سند قبض مرتبط بخدمة العمرة</h3><p>لا يتم تحديث مدفوعات الخدمة إلا بعد ترحيل السند المرتبط بها.</p></div></div>
      <form className="form-grid" onSubmit={collect}>
        <Select label="الخدمة" value={receipt.booking_id} onChange={v=>setReceipt({...receipt,booking_id:v})} options={bookings.filter(b=>postedStatus(b.status)&&Number(b.remaining_amount)>0).map(b=>({value:String(b.id),label:`#${b.id} — المتبقي ${money(b.remaining_amount)}`}))}/>
        <label><span>رقم السند اليدوي (اختياري)</span><input value={receipt.voucher_number} onChange={e=>setReceipt({...receipt,voucher_number:e.target.value})} placeholder="اتركه فارغًا للترقيم التلقائي" /></label>
